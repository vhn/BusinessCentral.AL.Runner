...
//
//     CLEAN (6, trip-wires)          BROKEN (3, RED until the widening rule lands)
//     ------------------------       --------------------------------------------
//     SourceTable                    PageExtension.TargetObject .. AL0270 the control
//     CalcFormula                        'BystanderMarker' is not found in the target
//     RunObject                      report RelatedTable ......... AL0118 the name
//     LookupPageId/DrillDownPageId       'Description' does not exist in the current context
//     enum-value Implementation      query RelatedTable .......... AL0386 a required package
//     RoleCenter                         dependency could not be found
//
...
//
// query RelatedTable is a THIRD thing and is NOT explained. AL0386 is a package-resolution
// failure, not a by-name break, and it is the same diagnostic the original probe hit — which was
// attributed there to the probe putting the query IN the delta. Here the query is the bystander
// and is never code-generated by the delta, so that explanation does not hold and the real cause
// is unknown. Cold is clean, so the divergence is real; do not fold it into the widening story
...
// about it — including its by-name references to the object that was just stripped. Six of the
// nine survive that narrowness; three do not. Nothing was pinning any of them.
//
...
// path, and pointing its dataitem at a missing table proves the reference is really bound — but
// see the header: it diverges for a reason that is not a by-name break, and is unresolved.
...
public sealed class RadByNamePropertyShapesTests(BcEngineFixture engine)
{
    ...
    public void ReportRelatedTable_SurvivesStrippingTheTableItNames()
    {
        ...
        Shape(
            ...
            modified: ["BN Report Host Ext", "BN Report Table"],
            emitted: ["BN Report Host Ext", "BN Report Table"]);
    }
    ...
    public void PageExtensionTargetObject_SurvivesStrippingThePageItNames()
    {
        ...
        Shape(
            ...
            modified: ["BN PageExt Caller", "BN PageExt Page"],
            emitted: ["BN PageExt Caller", "BN PageExt Page"]);
    }
    ...
    public void QueryRelatedTable_SurvivesStrippingTheTableItNames()
    {
        ...
        Shape(
            ...
            modified: ["BN Query Caller", "BN Query Table"],
            emitted: ["BN Query Caller", "BN Query Table"]);
    }
